namespace Quantum
{
	...
	/// </summary>
	public unsafe class FighterEnvironmentProcessor : KCCProcessor, IBeforeMove, IAfterMoveStep
	{
		...
		[KCCHeader("Other")] 
		public FP ExternalCollisionStrength = 1;
		...
		public static void SetDynamicVelocity(KCCContext context, ref KCCData data, FP jumpMultiplier, FP groundFriction, FP airFriction, FP externalCollisionStrength, FPVector3 GroundSnapGravity)
		{
			...
			dynamicVelocity += data.ExternalCollisionImpulse * externalCollisionStrength;
				...
				else
				{
					...
					if (data.IsGrounded == true)
					{
						...
						dynamicVelocity += KCCPhysicsUtility.GetFriction(dynamicVelocity, dynamicVelocity,
							frictionAxis, data.GroundNormal, data.KinematicSpeed, true,
							FP._0, FP._0, groundFriction, deltaTime);
					}
					else
					{
						dynamicVelocity += KCCPhysicsUtility.GetFriction(dynamicVelocity, dynamicVelocity,
							new FPVector3(FP._1, FP._0, FP._1), data.KinematicSpeed, true,
							FP._0, FP._0, airFriction, deltaTime);
					}
				}
			}
			...
			data.ExternalCollisionImpulse = default;
		}
		...
		public static void SetKinematicVelocity(KCCContext context, ref KCCData data, FP speed)
		{
			...
			//Draw.Ray(data.BasePosition + new FPVector3(0, FP._0_33, 0), data.KinematicVelocity * FP._5, ColorRGBA.Black);
			if (data.IsGrounded == true)
			{
				...
				if (IsAlmostZero(data.KinematicDirection, FP.EN4) == false && KCCPhysicsUtility.ProjectOnGround(data.GroundNormal, data.KinematicDirection, out FPVector3 projectedMoveDirection) == true)
				{
					...
					//Draw.Ray(data.BasePosition + new FPVector3(0, FP._0_10, 0), data.KinematicTangent * FP._3, ColorRGBA.Red);
				}
				else
				{
					// Otherwise use ground tangent => steepest descent.
					data.KinematicTangent = data.GroundTangent;
					//Draw.Ray(data.BasePosition + new FPVector3(0, FP._0_10, 0), data.KinematicTangent * FP._3, ColorRGBA.Blue);
				}
			}
			else
			{
				...
				//Draw.Ray(data.BasePosition + new FPVector3(0, FP._0_75, 0), data.KinematicDirection * FP._5, ColorRGBA.Yellow);
			}
			...
			if (data.IsGrounded == true)
			{
				...
				{
					//Draw.Ray(data.BasePosition + new FPVector3(0, FP._0_50, 0), projectedKinematicVelocity.Normalized * FP._3, ColorRGBA.Green);
